...
export default function SFU(){
  ...
  const customStyles = {
    ...
    return(
      ...
      <div>
         <button onClick={() => setIsOpen(true)} className="bg-red-900 hover:bg-red-400 text-white  font-bold px-5 py-2 rounded-full">**Disclaimer**</button>
         <Modal isOpen={isOpen} onRequestClose={() => setIsOpen(false)} style={customStyles}>
            <h1>
              <p className='text-black'>The linked pages were built using Bootstrap<br></br>
            For best viewing exeperience, use a PC/Laptop/Tablet </p></h1>
            <button onClick={() => setIsOpen(false)} className='text-neutral-50 bg-red-500 rounded-md px-1  hover:bg-green-400'>Ok</button>
         </Modal>
      </div><br></br><br></br>
      <div className="px-40">
        <article className="font-serif text-neutral-50 bg-cyan-900 px-20 py-10 rounded-lg flex flex-col items-center justify-between text-md">
          <p>
